<master>
  ...
  <p>
    The MailDir or NaviServer's nsimap IMAP4 client module collects the email.
    If scanning a MailDir, email is checked in the directory specified by
    package parameter IncomingMailDir.
  </p><p>
    ...
    It strips <code>message_id</code> and verifies the hashkey from replied or bounced emails.
    </p><p>
    New email can be processed by setting the package parameter <code>IncomingFilterProcName</code> to
    the name of a custom filter that examines each email and assigns a
    <code>package_id</code> based on custom criteria.
  </p><p>
    Then, the procedure parses each email flagged for final parsing.

  </p><p>
    Then it logs events that trigger any relevant callback registered by the package associated with each email.
    This enables each package to deal with incoming email in its own way.
    For example, a social networking site could log an event in a special table about subscribers.
    ...
    A separate process runs daily that checks if an email account
  ...
  <ul>
    <li>If a user's last mail bounced more than
      ...
      ACS Mail Lite assumes the user's email account is working again and no longer
      refusing emails.
    ...
    <li>To notify users that they will not receive any more email and to
      ...
      a notification email is sent every <code>NotificationInterval</code> days 
      up to <code>MaxNotificationCount</code> times.
