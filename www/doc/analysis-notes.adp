...
<p>FastPriorityPackageIds and SlowPriorityPackageIds FastPriorityPartyIds and SlowPriorityPartyIds and StandardPriorityMin and StandardPriorityMax are defined in acs_maile_lite_ui, so they can be tuned without restarting server. ps. Code should check if user is banned before processing.</p>
